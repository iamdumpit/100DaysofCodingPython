...
def init():
    ...
    if choice_diff < 0 or choice_diff > 3:
        print("Print choose the correct difficulty next time. Type 0 = Easy, 1 = Medium, 2 = Hard, 3 = Geek Mode!")
    else:
        clear()
        ...
        while game_ends != True:
            try:
                guess = input(f"You need to guess the {len(chosen_word)} letter word\n").upper()
